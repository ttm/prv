...
positions) is in the \tttt{tf.hsyls} variable after executing the \tttt{basic/stardardAll.py}
script.
...
specially the \tttt{tokipona/syntax/tokipona.vim} syntax file

Currently, the Python package synthesizes the
syntax file through the\\ \tttt{syntax.TPSynHigh} class.
...
\begin{itemize}
  ...
  \item Instantiating the \tttt{syntax.TPSynHigh} class and then executing the \tttt{mkSynHighFile()} method
  ...
  \item Access the used highlighted groups with \tttt{:syntax}.
    ...
    This may also be achieved changing \tttt{syntax.TPSynHigh().colors}
    attribute and then executing \tttt{mkSynHighFile()}.
\end{itemize}

\noindent Such procedures are exemplified in \tttt{basic/syntaxhighlighting.py}.

The syntax file has an association of TP
tokens with highlighting groups (HIGs).
It also holds associations of these HIGs to other HIGs 
to use their color settings.
Thus, one may change: associations of tokens to HIGs;
